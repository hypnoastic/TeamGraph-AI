...
function graphNodes(input: GraphVisualization["nodes"]): Node[] {
  return input.map((node, index) => ({
    id: node.id,
    position: { x: 80 + (index % 4) * 240, y: 80 + Math.floor(index / 4) * 150 },
    data: { label: node.label, type: node.type, meta: node.meta || {} },
    style: { background: palettes[node.type] || "#FFFDF8", border: "2px solid #111", borderRadius: 2, boxShadow: "5px 5px 0 #111", color: "#111", fontWeight: 800, width: 190, padding: 14 },
  }));
}

function graphEdges(input: GraphVisualization["edges"]): Edge[] {
  return input.map((edge) => ({ ...edge, style: { stroke: "#111", strokeWidth: 2 }, labelStyle: { fill: "#111", fontWeight: 700, fontSize: 10 }, markerEnd: { type: MarkerType.ArrowClosed, color: "#111" } }));
}
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_310px]">
        ...
        <aside className="space-y-5">
          <section className="panel p-4">
            ...
            {selected ? <div className="pt-4"><span className="badge" style={{ background: palettes[String(selected.data.type)] }}>{String(selected.data.type)}</span><h3 className="mt-3 text-xl font-black">{String(selected.data.label)}</h3>{Object.entries(meta).slice(0, 6).map(([key, value]) => <div key={key} className="mt-3 border-t border-black/30 pt-2 text-xs"><b>{key}</b><div className="mono mt-1 break-all text-[10px]">{String(value)}</div></div>)}</div> : <div className="empty-state mt-4">Select a node.</div>}
          </section>
          <section className="panel max-h-80 overflow-y-auto p-4">
            <h2 className="mb-3 font-black">Timeline</h2>
            {timeline.slice(0, 8).map((item) => <button key={item.id} className="block w-full border-t border-black/30 py-3 text-left"><b className="text-sm">{item.title}</b><div className="mono text-[10px]">{item.projectName || "Organization"}</div></button>)}
          </section>
        </aside>
      </div>
    </PageShell>
