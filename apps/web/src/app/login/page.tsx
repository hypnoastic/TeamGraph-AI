      ...
      <section className="grid place-items-center bg-[var(--paper)] p-5">
        <GoogleOAuthProvider clientId={process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || ""}>
          <form onSubmit={authenticate} className="panel w-full max-w-md p-6 md:p-8">
            ...
            <div className="grid grid-cols-2 gap-2">{(["admin", "member"] as const).map((item) => <button type="button" className="btn-secondary min-h-9 px-2 text-xs capitalize" key={item} onClick={() => demo(item)}>{item}</button>)}</div>
          </form>
        </GoogleOAuthProvider>
      </section>
    </main>
